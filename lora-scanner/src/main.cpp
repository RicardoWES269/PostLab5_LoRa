...
void setup() {
  ...
  int16_t state = radio.begin(904.9, 125, 10, 5, 0x34, 0, 8);
  if (state != RADIOLIB_ERR_NONE) {
...
void loop() {
  ...
  if (buttonFlag) {
    ...
    // transmit packet
    Serial.print("Button pressed! Switching channels...");
    advance_channel();
    //int16_t state = radio.transmit("CSE122/CSE222C/WES269 - Hello World!");
    //if (state == RADIOLIB_ERR_NONE) {
    //  Serial.println("Complete!");
